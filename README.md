...
**对话选股**：用大白话描述选股想法，筛选结果直接以表格呈现，命中的条件写在结果上方。

![对话选股](assets/screenshot-chat.png)

**自选策略**：把常看的条件存成策略，之后在对话里按名字调用；支持试跑、编辑、回测。

![自选策略](assets/screenshot-strategies.png)

**策略回测 + 回测可信度**：先给可信度清单，再给数字。下图这条示例策略跑输基准，工具如实标注「没有产生超额价值」并给出「偏低」评级——回测结果不好看就写成不好看。

![策略回测与可信度](assets/screenshot-backtest.png)

**知识库**：导入自己的选股资料（策略、规则、经验、术语），回答时引用并注明出处；知识库为空时不给出任何建议。

![知识库](assets/screenshot-kb.png)

**日线走势**：点结果表任意一行查看个股 K 线。
